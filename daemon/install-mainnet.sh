...
echo "Installing Aztec Butler daemon (mainnet only)..."
...
# Generate service file dynamically (with --network mainnet)
echo "Generating service file..."
cat > "$SERVICE_FILE" <<'EOF_HEADER'
[Unit]
Description=Aztec Butler - Prometheus monitoring for Aztec mainnet
...
ExecStart=$NODE_PATH $INSTALL_DIR/dist/index.js serve --network mainnet
Restart=always
RestartSec=5
...
echo ""
echo "✅ Aztec Butler installed and started (mainnet only)"
echo ""
