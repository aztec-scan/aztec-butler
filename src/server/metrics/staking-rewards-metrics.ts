...
export const initStakingRewardsMetrics = () => {
  pendingRewardsGauge = createObservableGauge("staking_rewards_pending_units", {
  ...
  pendingRewardsGauge.addCallback((observableResult) => {
    ...
    for (const [network, state] of networkStates.entries()) {
      const data = state.stakingRewardsData;
      if (!data || data.size === 0) {
        continue;
      }
  ...
  earnedCounter.addCallback((observableResult) => {
    ...
    for (const [network, state] of networkStates.entries()) {
      const history = state.stakingRewardsHistory;
      ...
      perCoinbase.forEach((ctx, coinbase) => {
        observableResult.observe(toNumber(ctx.totalEarned), {
          network,
          coinbase,
        });
      });
