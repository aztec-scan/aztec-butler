 ...
 */
export const getAttesterCoinbaseInfo = (): Map<string, string> => {
  const coinbaseMap = new Map<string, string>();

  if (!appState.scraperConfig) {
    return coinbaseMap;
  }

  for (const attester of appState.scraperConfig.attesters) {
...
export const initAttesterStatesFromScraperConfig = (
  ...
  console.log("[State] Initializing attester states from scraper config...");
...
export const updateScraperConfigState = (
  ...
  appState.scraperConfig = scraperConfig;
  console.log("[State] Scraper config stored in state");
};

/**
 * Get scraper config from state
 */
export const getScraperConfig = (): ScraperConfig | null => {
  return appState.scraperConfig;
};
