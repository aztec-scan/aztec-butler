...
const BN254_SCALAR_FIELD_ORDER =
  21888242871839275222246405745257275088548364400416034343698204186575808495617n;

const parseBlsSecretKey = (value: string, validatorIndex: number): bigint => {
  let secretKey: bigint;
  try {
    secretKey = BigInt(value);
  } catch {
    throw new Error(
      `Validator ${validatorIndex}: attester.bls must be a BN254 BLS private key scalar. Received a non-numeric value.`,
    );
  }

  if (secretKey <= 0n || secretKey >= BN254_SCALAR_FIELD_ORDER) {
    throw new Error(
      `Validator ${validatorIndex}: attester.bls must be a BN254 BLS private key scalar in the range 1 <= n < ${BN254_SCALAR_FIELD_ORDER}. ` +
        `The value looks like a public/compressed BLS key, which cannot be used to generate G2 public key and proof-of-possession registration data. ` +
        `Pass the original private key file to add-keys, not the public keys file generated by process-private-keys.`,
    );
  }
...
const command = async (
  ...
  const blsSecretKeys = keystore.data.validators.map((validator, index) =>
    parseBlsSecretKey(validator.attester.bls, index),
  );
  ...
  if (options.registry === "olla") {
    console.log(
      `\n${stakingProviderData.providerId} - Admin: ${stakingProviderData.admin}, Rewards Recipient: ${stakingProviderData.rewardsRecipient}`,
    );
  } else {
    console.log(
  ...
  const keyStores = [];
  for (const [index, validator] of keystore.data.validators.entries()) {
